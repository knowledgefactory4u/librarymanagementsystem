...
public class Mapper {
	...
	public static List<BookRecord> bookModelToVo(List<Book> books) {
		...
		return books.stream().map(vo -> new BookRecord(vo.getId(), vo.getIsbn(), vo.getName(), vo.getSerialName(),
				vo.getDescription())).toList();
	}

	public static List<AuthorRecord> authorModelToVo(List<Author> authors) {

		return authors.stream().map(vo -> new AuthorRecord(vo.getId(), vo.getName(), vo.getDescription())).toList();

	}

	public static List<CategoryRecord> categoryModelToVo(List<Category> categories) {

		return categories.stream().map(vo -> new CategoryRecord(vo.getId(), vo.getName())).toList();

	}

	public static List<PublisherRecord> publisherModelToVo(List<Publisher> publishers) {

		return publishers.stream().map(vo -> new PublisherRecord(vo.getId(), vo.getName())).toList();
